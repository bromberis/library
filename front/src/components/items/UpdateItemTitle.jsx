...
import { findItemAndUpdate } from "../../api/library/UsersApi";
import { useGlobalUserContext } from "../context/UserContext";
import { AiOutlineCheck, AiOutlineClose } from "react-icons/ai";

function UpdateItemTitle(props) {
  const { userData, refreshUserData } = useGlobalUserContext();

  const { itemID, setIsEditing, title, category, date } = props;
  const {
    register,
    handleSubmit,
    reset,
  ...
  function onSubmit(data) {
    ...
    findItemAndUpdate(newObj, userData._id, itemID).then(() => {
